...
;; query-system needs data-directed programming system
  ...
  (let ((q (query-syntax-process (read))))
    (cond ((assertion-to-be-added? q)
	  ...
	  (else
	   ...
	   (newline)
	   (display-stream
	   ;;(write-stream ;;it's write only characters.
	    (stream-map
	     (lambda (frame)
	       (instantiate q
			    frame
...
;; lisp-value
;; any procedure of scheme (e.g. <, sum-procedure)
...
;; 本来extendの定義は4.4.4.8だが，gaucheでは組み込みのextendがあり，エラーが出るため
